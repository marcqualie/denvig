...
- `DenvigProject.slug` and `.id` are now derived from `projectRefs()` via the new sync `projectSlug()` and `projectId()` helpers (`src/lib/project/refs.ts`). The legacy async `getProjectSlug()` in `src/lib/git.ts` has been removed; callers in `src/lib/project.ts` and `src/lib/projects.ts` now use the new sync helper. The github-remote fallback (preferring a dedicated `[remote "github"]` URL when `origin` doesn't point at github.com) is preserved.
- `DenvigProject` now exposes a `refs: string[]` getter returning the full list of `projectRefs()` for the project. The refs are computed once in the constructor and reused for `.slug`, `.id`, and `.refs`. `getProjectInfo()` now reads `project.refs` directly instead of recomputing. The SDK's `projects.list()` already returns `refs` on each `ProjectResponse` since the type is `Omit<ProjectInfo, 'serviceStatus'>`.
- **Breaking:** `projectId()` is now the SHA1 hash of `{ path, gitRemote, gitWorktree }` (via the `id:` ref) instead of just the absolute path. This changes the IDs used in launchctl service labels (`denvig.<id>.<service>`) and `~/.denvig/services/<id>.<service>/` directories. Services started before this change will have stale labels/paths and should be uninstalled and reinstalled, or migrated by hand.
...
- `deps outdated --release-latency` flag to filter out recently-released versions (e.g. `--release-latency 7d`).
  - Defaults to `auto`, which reads pnpm's `minimumReleaseAge` from `pnpm-workspace.yaml` when available
  - respects pnpm's `minimumReleaseAgeExclude` list, allowing excluded packages to bypass the latency filter
...
- `DENVIG_OPEN_VIA` environment variable to control version link target: `npm` (default), `npmx`, or `none` to disable links
